...
### 2026-07-01 - 产品定位聚焦音乐可视化播放器

- 用户认可/要求保留：Mineradio 本质是可视化音乐播放器，不需要多余的外围功能堆叠；重点是音乐可视化表达、播放体验、肉眼可见的丝滑操作和流畅 UI。
- 涉及文件：`public/index.html`，后续涉及 Home、播放控制、歌单、搜索和视觉系统。
- 关键参数/实现：Home 卡片与快捷入口优先围绕播放、歌单、每日/推荐队列和视觉体验；已登录哪个平台，就优先显示/搜索/播放哪个平台的歌单与音源，不让其他平台无故介入。
- 禁止回退或改坏的点：不要把 Home 做成杂项入口集合；不要让点击多个主界面卡片都只弹侧边栏；不要在用户只登录酷狗时混入网易云/QQ 歌单或音源导致试听/未登录误判；交互必须清晰、快速、连贯。
